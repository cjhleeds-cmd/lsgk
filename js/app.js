...
const override=provConfig[mapKey]?.[unitIndex];
if(override!==undefined){configured=override===null?undefined:override;}
}
...
const mapKeys=['gangyao-shang','gangyao-xia','xuanbi1','xuanbi2','xuanbi3'];
const mapKey=mapKeys[mapIndex];
const unitScenes=provConfig[mapKey]?.[unitIndex];
if(unitScenes){ids=unitScenes;}
}
return ids.map(id=>typeof TEACH_SCENES!=='undefined'?TEACH_SCENES[id]:null).filter(Boolean);
}

// 获取当前省份配置的所有历史现场场景ID（去重）
// 规则：如果省份有独立的场景映射配置，则只使用省份配置的场景（各省独立）
//      如果省份没有独立配置，则使用默认配置的场景
...
if(provConfig){
  ...
  for(let mapIndex=0;mapIndex<mapKeys.length;mapIndex++){
    const mapKey=mapKeys[mapIndex];
    const mapScenes=provConfig[mapKey];
    if(!mapScenes) continue;
    for(const unitIndex in mapScenes){
      const unitSceneIds=mapScenes[unitIndex];
      if(Array.isArray(unitSceneIds)){
        unitSceneIds.forEach(id=>ids.add(id));
      }
    }
  }
}else{
  // 省份没有独立配置：使用默认配置的场景
  for(let mapIndex=0;mapIndex<mapKeys.length;mapIndex++){
    const cfg=getGameMapConfig(mapIndex);
    if(!cfg||!cfg.enabled) continue;
    const units=cfg.units||[];
    for(let i=0;i<units.length;i++){
      const sceneIds=units[i].sceneIds||[];
...
const playableIndexes=map.units.map((unit,index)=>(unit.questions?.length||getUnitScenes(i,index).length)?index:-1).filter(index=>index>=0);
...
if(!unit.questions?.length&&!getUnitScenes(mapIndex,unitIndex).length)return;
totalU+=1;
...
const provinceSceneIds=getProvinceSceneIds();
const sceneCount=provinceSceneIds.length;
...
if(!units[unitIdx]?.questions?.length){
const scenes=getUnitScenes(mapIdx,unitIdx);
if(scenes.length) return 'scene-only';
return 'empty';
}
...
const playableIndexes=map.units.map((unit,index)=>(unit.questions?.length||getUnitScenes(state.currentMap,index).length)?index:-1).filter(index=>index>=0);
const completed = playableIndexes.filter(index=>p.units[index]?.completed).length;
...
const provinceSceneIds=getProvinceSceneIds();
const provinceScenes=provinceSceneIds.map(id=>typeof TEACH_SCENES!=='undefined'?TEACH_SCENES[id]:null).filter(Boolean);
const mapSceneCount=provinceScenes.filter(scene=>scene.category===mapFilter).length;
const mapSceneStampCount=provinceScenes.filter(scene=>scene.category===mapFilter&&teachStamps[scene.id]).length;
...
const countSuffix=(status==='empty'||status==='scene-only')?'':(quizQuestionCount>=focusQuestionCount?` · 重点${quizQuestionCount}题`:quizQuestionCount<3?` · ${quizQuestionCount}题`:'');
const statusText = (status==='empty'?'暂无题目':status==='scene-only'?'历史现场':status==='completed'?(gameMap?'长河已修复':'已完成'):'当前可学')+countSuffix;
const statusClass=status==='empty'?'locked empty':status==='scene-only'?'scene-only':status;
...
html += `<button type="button" class="level-node ${statusClass}${fractureCount?' has-fracture':''}" onclick="${status==='scene-only'?`enterUnitScene(${i})`:`enterQuiz(${i})`}" data-status="${status}" aria-label="第 ${i+1} 关 ${escapeHtml(unit.name)}，${statusText}" ${status==='empty'?'disabled':''}>`;
...
html += `<div class="node-copy"><div class="node-card-meta"><span class="node-status">${statusText}</span><div class="node-stars" aria-label="${stars} 星">${starsHtml}</div></div><h3 class="node-label">${escapeHtml(unit.name)}</h3>${unitScenes.length?`<div class="node-reward"><span class="${(status==='completed'||status==='scene-only')?'unlocked':'locked'}">${unitScenes.length}个历史情境</span>${latestStampedScene?`<b class="node-achievement-badge" aria-label="${escapeHtml(latestStampLabel)}历史印章" title="最近获得：${escapeHtml(latestStampLabel)}历史印章"><span>${escapeHtml(latestStampLabel)}</span><i aria-hidden="true">印</i></b>`:`<em>${(status==='completed'||status==='scene-only')?'已开放':'未通关'}</em>`}</div>`:''}</div>`;
html += `</button>`;
});
...
const provinceSceneIds=getProvinceSceneIds();
const scenes=provinceSceneIds.map(id=>typeof TEACH_SCENES!=='undefined'?TEACH_SCENES[id]:null).filter(Boolean);
...
function enterUnitScene(unitIdx){
const scenes=getUnitScenes(state.currentMap,unitIdx);
if(!scenes.length){
alert('这一关暂时没有历史现场。');
return;
}
...
if(scenes.length===1){
enterTeach(scenes[0].id,'map');
}else{
const mapFilter=getGameMapConfig(state.currentMap)?.filter||'all';
openTeachCatalog(mapFilter,'map');
}
}
...
function switchProvince(province){
  if(!province || province === window.__PROVINCE) return;
  location.href = '历史长河.html?province=' + encodeURIComponent(province);
}
