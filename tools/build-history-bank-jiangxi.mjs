...
const bankFile = path.join(root, 'data', 'provinces', 'jiangxi.js');
const configFile = path.join(root, 'tools', 'paper-config-jiangxi.json');
const letters = 'ABCD';
...
for (const paperConfig of config.papers) {
  const sourcePath = path.join(root, 'documents/试卷/jiangxi', paperConfig.file);
  ...
  const archivedQuestions = parsed.questions.map(rawQuestion => {
    ...
    if (!location) {
      throw new Error(`${paperConfig.file} 第 ${rawQuestion.number} 题（选择题序号${index + 1}）缺少归类。`);
    }
...
const output = `/*
 * 历史长河题库 — 江西省
 * 由 tools/build-history-bank-jiangxi.mjs 维护。
 * 练习题只取自 documents/试卷/jiangxi/ 中的试卷。
