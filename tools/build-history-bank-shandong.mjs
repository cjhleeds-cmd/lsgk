...
const bankFile = path.join(root, 'data', 'provinces', 'shandong.js');
const configFile = path.join(root, 'tools', 'paper-config-shandong.json');
const letters = 'ABCD';
...
function parsePaper(file) {
  ...
  const questions = unique.map((heading, index) => {
    ...
    // Extract original explanation if present
    const explMatch = body.match(/\*\*解析[：:]\*\*\s*([\s\S]*?)(?=\*\*(?:答案|解析)|---|$)/);
    const origExplanation = explMatch ? clean(explMatch[1]) : '';
...
/**
 * Extract per-option explanations from the original exam explanation text.
 * Returns an array of 4 strings (for A, B, C, D).
 * For the correct option: empty string (explanation field already covers it).
 * For wrong options: the specific reason extracted from the original explanation.
 */
function extractOptionExplanations(origExplanation, correctAnswer) {
  const result = ['', '', '', ''];
  if (!origExplanation) return result;

  const text = origExplanation;

  // Pattern 1: "X项正确" or "X正确" - identify correct option
  // Pattern 2: "排除X项" or "排除X" - wrong option, reason is text before
  // Pattern 3: "X项错误" or "X错误" - wrong option, reason is text before
  // Pattern 4: "故选X项" - just confirms correct answer

  // Split by option markers and extract reasons
  // Look for patterns like "XXX，A项错误" or "XXX，排除A项" or "XXX，排除A"
  const optionPattern = /(?:([A-D])项(?:正确|错误)|排除([A-D])项?|([A-D])(?:正确|错误))(?:[，。；]|$)/g;

  let matches = [];
  let match;
  while ((match = optionPattern.exec(text)) !== null) {
    const letter = match[1] || match[2] || match[3];
    const isCorrect = /正确/.test(match[0]);
    const optionIdx = letters.indexOf(letter);
    if (optionIdx < 0) continue;

    // Find the reason text before this marker
    // Look backwards from match.index to the previous marker or start
    let reasonStart = 0;
    for (const prevMatch of matches) {
      if (prevMatch.endPos > reasonStart) reasonStart = prevMatch.endPos;
    }
    const reasonText = text.slice(reasonStart, match.index).replace(/^[，。；、\s]+/, '').replace(/[，。；、\s]+$/, '');
  ...
  // If no matches found with the main pattern, try simpler patterns
  if (matches.length === 0) {
    // Try "排除X" without 项
    const simplePattern = /排除([A-D])/g;
  ...
  // Assign reasons to options
  for (const m of matches) {
    if (m.optionIdx === correctAnswer) {
      // Correct option - leave empty (explanation field covers it)
      result[m.optionIdx] = '';
    } else if (m.reason) {
      // Wrong option - use the extracted reason
      result[m.optionIdx] = m.reason;
    }
  }

  // For any wrong options without extracted reasons, try to find any relevant text
  for (let i = 0; i < 4; i++) {
    if (i === correctAnswer) continue;
    if (!result[i]) {
      // Try to find the option letter mentioned in context
      const letter = letters[i];
      const letterPattern = new RegExp(`${letter}项[^，。；]*`, 'g');
      const letterMatch = letterPattern.exec(text);
      if (letterMatch) {
        result[i] = letterMatch[0].replace(/^[A-D]项/, '').replace(/[，。；\s]+$/, '');
      }
    }
  }
...
// Map styling data (consistent with Guangdong and Hebei)
const MAP_STYLES = [
  { icon: '📜', bgClass: 'map-bg-ancient', nodeClass: 'map-ancient', pathColor: '#C4956A', pathColorLight: 'rgba(196,149,106,.25)' },
  { icon: '🌍', bgClass: 'map-bg-world', nodeClass: 'map-world', pathColor: '#6B8E9F', pathColorLight: 'rgba(107,142,159,.25)' },
  { icon: '⚖️', bgClass: 'map-bg-modern', nodeClass: 'map-modern', pathColor: '#8B9A6B', pathColorLight: 'rgba(139,154,107,.25)' },
  { icon: '🏭', bgClass: 'map-bg-ancient', nodeClass: 'map-ancient', pathColor: '#A080A0', pathColorLight: 'rgba(160,128,160,.25)' },
  { icon: '🎭', bgClass: 'map-bg-world', nodeClass: 'map-world', pathColor: '#D4A060', pathColorLight: 'rgba(212,160,96,.25)' }
];
...
function addPracticeQuestion({ question, answer, reason, location, source }) {
  ...
  // Extract option explanations from original exam explanation
  const optionExplanations = extractOptionExplanations(question.origExplanation, answer);
...
for (const paperConfig of config.papers) {
  ...
  const archivedQuestions = parsed.questions.map(rawQuestion => {
    ...
    if (answer < 0) {
      // Question not in answers (excluded as chart-dependent)
      const visualReason = visualExclusionReason(rawQuestion);
      if (visualReason) {
        exclusions.push({ source: `${paperConfig.label}·第${rawQuestion.number}题`, type: 'visual', reason: visualReason });
        return {
    ...
    // Find the correct location and reason for this question
  ...
  papers.push({
    ...
    answerStatus: paperConfig.answerStatus || '原始Markdown附答案与解析',
...
const output = `/*
 * 历史长河题库 — 山东省
 * 由 tools/build-history-bank-shandong.mjs 维护。
 * 练习题只取自 documents/试卷/shandong/ 中的试卷。
