...
const bankFile = path.join(root, 'data', 'provinces', 'hebei.js');
const configFile = path.join(root, 'tools', 'paper-config-hebei.json');
const letters = 'ABCD';
...
for (const paperConfig of config.papers) {
  ...
  const archivedQuestions = parsed.questions.map(rawQuestion => {
    ...
    // Since chart questions are excluded from answers/locations/reasons arrays,
    // we need to map question numbers to array indices
...
const output = `/*
 * 历史长河题库 — 河北省
 * 由 tools/build-history-bank-hebei.mjs 维护。
